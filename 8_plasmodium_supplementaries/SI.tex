...
{\emph{P. falciparum} libraries are presented in this work and
    \emph{S. cerevisiae} libraries from Duan et al.~\cite{duan:three-dimensional}
...
\multirow{4}{*} {\emph{S. cerevisiae}~\cite{duan:three-dimensional}} & HindIII-MspI	 & 	1.92	 & 	8.99\% \\
...
\begin{figure}
  ...
  \begin{tabular}{llcccc}
    ...
    \hline
    {Yeast {\small \textit{(Hi-C)}}} &Yeast \textit{(VE)} & 0.915 & 0.573 \\
    {\small \citep{duan:three-dimensional}} & Yeast {\small \textit{(expected)}} & 0.922 & 0.115 \\\hline
    \multirow{2}{*}{Ring {\small \textit{(Hi-C)}}} &Ring \textit{(VE)} & 0.843 & 0.340 \\
    & Ring {\small \textit{(expected)}} & 0.928 & 0.072 \\\hline
    \multirow{2}{*}{Trophozoite {\small \textit{(Hi-C)}}} &Trophozoite \textit{(VE)} & 0.848 & 0.392 \\
    & Trophozoite {\small \textit{(expected)}} & 0.908 & 0.063 \\\hline
    \multirow{2}{*}{Schizont {\small \textit{(Hi-C)}}} &Schizont \textit{(VE)} & 0.864 & 0.487 \\
    & Schizont {\small \textit{(expected)}} & 0.923 & 0.081 \\
    \hline
    \end{tabular}}
  \end{center}
...
%For calculating p-values of group 1, we use a power-law fit which is of the
%form $y = c x^{k} + \epsilon$ where $y$ is the probability that a given
%paired-end read is between two loci that have a genomic distance of $x$
%between them and $\epsilon$ is the residual. It has been shown in the
%literature (Lieberman-Aiden et al) that this functional form is a nice fit
%and captures the exponential decrease in contact probability with genomic
%distance for cross-linked libraries. We are using a current tool we developed
%(fit-hic) which calculates the best power-law fit (or spline) as a background
%and finds the pairs of loci that are interacting significantly more compared
%to this background to set them aside as outliers in the first phase. Using
%the non-outlier pairs it re-calculates the best fit which is a better
%representation of the background contact probability. Then, using this second
%fit as the background it computes p-values for each intra proximal pair.
...
A monolayer of parasites was deposited within a 9 $\times$ 9 mm frame-seal
